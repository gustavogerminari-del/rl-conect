...
export const MasterAdminView: React.FC<MasterAdminViewProps> = ({ initialSection = 'dashboard' }) => {
  ...
  return (
    <div className="-m-4 flex min-h-screen flex-col bg-slate-950 text-slate-100 sm:-m-6 lg:-m-8">
        ...
        <main className="min-w-0 flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
          ...
          {loading ? <LoadingState label="Carregando dados reais do Firebase..." compact /> : (
            <MasterSectionErrorBoundary section={activeSection} onGoHome={() => setActiveSection('dashboard')}>
              ...
              {activeSection === 'usuarios' && <UsersSection users={users} tenants={tenants} onNew={() => setFormModal('user')} onEdit={setEditingUser} onReset={async (user: UserProfile) => { await UserService.sendPasswordReset(user.email); window.alert(`E-mail de redefinição enviado para ${user.email}. Você não tem acesso à nova senha do cliente.`); }} onToggle={async (user: UserProfile) => { const role = String(user.role || '').toUpperCase().replace(/[\s-]+/g, '_'); if (role === 'MASTER' || role === 'MASTER_ADMIN' || user.tipoUsuario === 'MASTER') return; await UserService.update(user.uid, { status: user.status === 'Ativo' ? 'Bloqueado' : 'Ativo' }); await loadData(); }} />}
      ...
      {editingUser && <UserAccessEditModal user={editingUser} tenants={tenants} modules={modules} saving={saving} onClose={() => setEditingUser(null)} onSave={async (payload) => { setSaving(true); try { await UserService.update(editingUser.uid, payload); setEditingUser(null); await loadData(); } finally { setSaving(false); } }} />}
    </div>
  );
};
...
function UsersSection({ users, tenants, onNew, onEdit, onReset, onToggle }: any) { return <><SectionHeader title="Usuários e Permissões" description="Edite empresa, perfil e permissões sem conhecer a senha do cliente. Senhas são tratadas exclusivamente pelo Firebase Authentication." action={<button onClick={onNew} className={primaryButton}><KeyRound className="h-4 w-4" />Criar acesso</button>} />{users.length ? <div className="overflow-x-auto rounded-2xl border border-slate-800"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-slate-900 text-slate-400"><tr><th className="p-3">Usuário</th><th>Empresa</th><th>Perfil</th><th>Permissões</th><th>Status</th><th>Ações</th></tr></thead><tbody>{users.map((user: UserProfile) => { const role = String(user.role || '').toUpperCase().replace(/[\s-]+/g, '_'); const protectedMaster = role === 'MASTER' || role === 'MASTER_ADMIN' || user.tipoUsuario === 'MASTER'; return <tr key={user.uid} className="border-t border-slate-800"><td className="p-3"><b className="text-white">{user.displayName}</b><div className="text-slate-400">{user.email}</div></td><td>{protectedMaster ? 'Plataforma RL Connect' : tenants.find((t: ClientTenant) => t.id === user.companyId)?.companyName || user.companyId}</td><td>{user.role}</td><td>{user.permissions?.length || 0}</td><td><Badge value={protectedMaster ? 'PROTEGIDO' : user.status} /></td><td><div className="flex flex-wrap gap-2">{!protectedMaster && <button onClick={() => onEdit(user)} className={secondaryButton}>Editar acesso</button>}<button onClick={() => onReset(user)} className={secondaryButton}>Redefinir senha</button>{protectedMaster ? <span className="self-center text-[10px] font-black uppercase text-emerald-300">Não pode bloquear</span> : <button onClick={() => onToggle(user)} className={secondaryButton}>{user.status === 'Ativo' ? 'Bloquear' : 'Reativar'}</button>}</div></td></tr>; })}</tbody></table></div> : <EmptyState />}</> }
...
function UserAccessEditModal({ user, tenants, modules, saving, onClose, onSave }: { user: UserProfile; tenants: ClientTenant[]; modules: PlatformModule[]; saving: boolean; onClose: () => void; onSave: (payload: Partial<UserProfile>) => Promise<void> }) {
